...
3-系统反馈引起的MT消息。限制SP不能对该字段填写3*/
        ...
        try {
            byte[] tempMsgBytes = MessageContent;
            if (TP_udhi == 1) {
                tempMsgBytes = new byte[MessageContent.length - 6];
                System.arraycopy(MessageContent, 6, tempMsgBytes, 0, MessageContent.length - 6);
            }
            // UCS2编码或GBK编码
            String msgStr = 8 == MessageCoding ? new String(tempMsgBytes, "ISO-10646-UCS-2") : 15 == MessageCoding ? new String(tempMsgBytes, "GBK") : new String(tempMsgBytes);
            outStr = String.valueOf(String.valueOf((new StringBuffer(String.valueOf(String.valueOf(outStr)))).append(",MessageContent=").append(msgStr)));

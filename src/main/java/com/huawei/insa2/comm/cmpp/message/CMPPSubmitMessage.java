...
public class CMPPSubmitMessage extends CMPPMessage
{
        ...
        throws IllegalArgumentException
    {
        ...
        try {
            byte[] tempMsgBytes = msg_Content;
            if (tp_Udhi == 1) {
                tempMsgBytes = new byte[msg_Content.length - 6];
                System.arraycopy(msg_Content, 6, tempMsgBytes, 0, msg_Content.length - 6);
            }
            // UCS2编码或GBK编码
            String msgContent = 8 == msg_Fmt ? new String(tempMsgBytes, "ISO-10646-UCS-2") : 15 == msg_Fmt ? new String(tempMsgBytes, "GBK") : new String(tempMsgBytes);
            outStr = String.valueOf(String.valueOf((new StringBuffer(String.valueOf(String.valueOf(outStr)))).append(",msg_Content=").append(msgContent)));
